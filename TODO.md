...
- 处理动作：按 RUNBOOK 将 `Dict.Query` 迁移为服务层 `DictQuery`，由 `DictInterfaceAssembler` 完成 `Request -> DictQuery`，移除当前 domain 已失效的 `Dict.Query/getQuery/setQuery`。
- 验收点：`DictController` 不再构造 `Dict.Query` 或直接搬运查询字段；`DictServiceImpl` 不再读取 `dict.getQuery()`；后台 API package 编译通过。
...
- 处理动作：按 RUNBOOK 将 `Menu.Query` 迁移为服务层 `MenuQuery`，由 `MenuInterfaceAssembler` 完成 `Request -> MenuQuery`，保持树查询和 `maxRank` 过滤语义，不改写移动、保存、删除动作。
- 验收点：`MenuApiController` 不再构造 `Menu.Query` 或直接搬运查询字段；`MenuServiceImplTest` 覆盖查询对象过滤；后台 API package 编译通过。
...
- 处理动作：按 RUNBOOK 将 `Role.Query` 迁移为服务层 `RoleQuery`，由 `RoleInterfaceAssembler` 完成 `Request -> RoleQuery`，同步处理 `UserApiController` 中角色列表读取调用。
- 验收点：后台 Controller 不再构造 `Role.Query` 或直接搬运查询字段；`RoleServiceImplTest` 覆盖启用状态过滤；后台 API package 编译通过。
...
- 处理动作：按 RUNBOOK 将 `User.Query` 迁移为服务层 `UserQuery`，由 `UserInterfaceAssembler` 完成 `Request -> UserQuery`，Controller 不再创建查询用空 `User`。
- 验收点：`UserApiController` 不再构造 `User.Query` 或直接搬运查询字段；新增或补齐 `UserServiceImplTest` 覆盖分页过滤；后台 API package 编译通过。
...
- 处理动作：按 RUNBOOK 将 `Office.Query` 迁移为服务层 `OfficeQuery`，由 `OfficeInterfaceAssembler` 完成 `Request -> OfficeQuery`，不扩展组织树移动和保存动作。
- 验收点：`OfficeApiController` 不再构造 `Office.Query` 或直接搬运查询字段；`OfficeServiceImplTest` 覆盖父级、名称、备注过滤；后台 API package 编译通过。
...
- 处理动作：按 RUNBOOK 将 `Log.Query` 迁移为服务层 `LogQuery`，由 `LogInterfaceAssembler` 完成 `Request -> LogQuery`，同步处理 `SysLogUtils` 中的查询条件创建。
- 验收点：`LogApiController` 不再构造 `Log.Query` 或直接搬运查询字段，`SysLogUtils` 不再构造 `Log.Query`；`LogServiceImplTest` 覆盖日志类型和时间过滤；后台 API package 编译通过。
...
- 处理动作：按 RUNBOOK 将 `Storage.Query` 迁移为服务层 `StorageQuery`，由 `StorageInterfaceAssembler` 完成 `Request -> StorageQuery`，不顺手删除 `StorageBusiness.Query`。
- 验收点：`StorageController` 不再构造 `Storage.Query` 或直接搬运查询字段；`StorageServiceImplTest` 覆盖分页过滤；后台 API package 编译通过。
...
- 处理动作：按 RUNBOOK 将 `SignatureService.page(String businessType, Page<Signature> page)` 收敛为 `SignatureQuery` 参数，由 `SignatureInterfaceAssembler` 完成 `Request -> SignatureQuery`，不扩展签名校验和删除动作。
- 验收点：`SignatureApiController` 通过 `SignatureInterfaceAssembler` 获取 `SignatureQuery` 并调用分页；`SignatureServiceImplTest` 覆盖 `businessType` 过滤；后台 API package 编译通过。
