...
- 为什么控制抽象新增
- 为什么收敛 common 工具层级
- 为什么拆分 common-mq、common-oss 和 common-test
- 为什么文档和测试只表达当前系统形态
- 为什么按单业务域应用建模
...
在引入 MyBatis-Plus、拆分 common 或删除 Mapper XML 之前，必须先判断现有模型边界是否干净。工具替换不能固化错误边界；如果 domain / Service 承载了持久化索引、分页方言或 SQL 维护算法，应先收敛边界，再替换工具。
...
消灭 Mapper XML 不等于消灭复杂性。复杂 SQL 不能因为去 XML 而被挤入 Controller、Service、大段注解 SQL 或脆弱 Wrapper。复杂性必须有明确的持久化实现归属，并保持三层边界可读、可验证。
...
`common-mq`、`common-oss` 和 `common-test` 是对稳定技术契约的拆分，而不是业务层级扩张。消息、对象存储和测试支撑都可能被多个入口或模块复用，但它们各自只承载通用技术语义：业务消息 topic、存储对象生命周期、对象引用关系和具体业务测试仍然归属业务模块、infra 或入口模块。

## 5. Current Shape Intent

Sandwich 的治理文档和 ArchUnit 测试只描述当前系统目标形态。

正式规则保留稳定形态和判断理由，不承载改造叙事、过渡方案或完成记录。

文档表达优先使用正向意图：说明当前代码应该如何命名、如何放置、如何分层、如何选择边界。负向表达只用于可以被 review、testcase、ArchUnit 或 checkstyle 验证的明确禁止项。

ArchUnit 测试守住当前形态。规则从当前设计推导，例如“按 ID 批量读取使用 `listByIds`”，不维护方法名、类名或包名的演进清单。

如果某个规则只能通过列举反例表达，应先补足正向命名、分层或边界规则，再决定是否需要 hard rule。

## 6. Single Domain Modeling Intent

Sandwich 当前应该按单业务域应用建模。数据归属应该通过具体业务身份表达，例如 `userId`、`departmentId`、`clientId` 或具体业务对象标识；只有产品明确进入多租户 SaaS 形态时，才引入租户模型和跨层租户隔离规则。
...
每次提交只收敛一个明确判断，是为了把变更半径压到最小，让定位、复盘和修正都落在可控范围内，而不是把多个判断混在一次大改中。

`commit message` 的意图不是重复“改了文件”，而是给这次判断建立可检索的工程记忆：

- 在时间上保留决策顺序，便于定位判断形成和规则收紧的位置。
- 在空间上标记改动落点，便于快速定位这是哪个模块、哪一层、哪一类问题。
- 在治理上把代码、测试、文档、`TODO.md` 清理绑定为同一条可解释的变更链路。

因此，AI 和开发者都不应把不相关修改堆叠到同一个提交里，也不应使用失去语义的 `commit message`。提交记录在本项目中承担的是“可回放的决策轨迹”，而不只是代码快照存档。
