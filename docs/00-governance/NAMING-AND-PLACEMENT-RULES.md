...
- `NAME_API_SERVICE_ENTRY_ONLY`：`sandwish-admin-api` 和 `sandwish-front-api` 中只有入口认证适配、注册编排、后台权限会话适配和后台系统日志消息服务类型允许使用 `Service` / `ServiceImpl` 后缀，且名称必须以 `AuthService`、`RegistrationService`、`PermissionService`、`SysLogMessageService` 或对应 `ServiceImpl` 结尾；后台系统日志消息服务按 `SysLogMessageService` / `SysLogMessageServiceImpl` 拆分；其他业务 Service 固定归属 `sandwish-biz`。
...
- 除入口专用认证适配、注册编排、后台权限会话适配 Service 和后台系统日志消息服务外，其他 Service interface 和 Service implementation 应归属 `sandwish-biz`；可复用认证业务 Service 优先归属 `sandwish-biz`，后台权限会话适配命名为 `PermissionService`，前台会员注册编排命名为 `MemberRegistrationService`，后台系统日志消息发送和消费服务命名为 `SysLogMessageService` / `SysLogMessageServiceImpl`
