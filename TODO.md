...
- [ ] `snowflake-doc-auth-db`：改文档，迁移认证数据库设计 ID 口径
  ...
  - 处理动作：将认证域数据库设计中的主键和关系字段调整为雪花 `Long` 口径。
  - 验收点：`AUTH-DATABASE-DESIGN.md` 不再声明 UUID 字符串主键默认规则。
  - 重要度：9/10
...
- [ ] `snowflake-doc-member-db`：改文档，迁移会员数据库设计 ID 口径
  ...
  - 处理动作：将会员域数据库设计中的主键和关系字段调整为雪花 `Long` 口径。
  - 验收点：`MEMBER-DATABASE-DESIGN.md` 不再声明 UUID 字符串主键默认规则。
  - 重要度：9/10
