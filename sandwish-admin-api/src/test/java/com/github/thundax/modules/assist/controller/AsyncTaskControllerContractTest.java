...
public class AsyncTaskControllerContractTest {
    ...
    public void shouldRejectPrivateTaskWhenCurrentUserDoesNotOwnIt() throws Exception {
        ...
        when(asyncTaskService.get(org.mockito.ArgumentMatchers.argThat(queryWithId(1001L))))
                .thenReturn(task);
        UserAccessHolder.currentUserId(2002L, "token-1");
    ...
    private org.mockito.ArgumentMatcher<AsyncTaskId> queryWithId(Long id) {
        return query -> query != null && AsyncTaskIdCodec.toDomain(id).equals(query);
    }

    private AsyncTask task(Long id) {
        AsyncTask task = new AsyncTask();
        task.setId(AsyncTaskIdCodec.toDomain(id));
    ...
    private void mockCurrentUser(Long userId) {
        ...
        currentUser.setId(UserIdCodec.toDomain(userId));
