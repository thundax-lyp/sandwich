...
- service 公开参数类型 `XxxQuery` / `XxxCommand` 默认放在 `*-service.ts`；如果同页面多个组件需要直接声明或组装该类型，提升到 `<domain>-types.ts`。
- service 返回给页面的业务数据类型 `XxxRecord` / `XxxNode` 默认放在 `*-service.ts`；如果同页面多个组件复用，或 service 文件过长，提升到 `<domain>-types.ts`。
