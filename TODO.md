...
1. 迁移 `Signature` 持久化链路
   - 以 `SignatureMapper.xml` 达梦实现为行为基准。
   - 迁出旧 CRUD 基类，展开查询 DAO 参数。
   - 仅 `insert` / `update` 使用 `Signature` 实体参数。
   - Mapper 保持最小 `BaseMapper<SignatureDO>` 定义，业务在 `DaoImpl` 中实现。
   - 用 `DaoImpl` 中的 MyBatis-Plus / Java 持久化实现替代 XML。
   - 消除当前链路 PageHelper 依赖，删除对应 XML，补齐最小验证。
2. 迁移 `Member` 持久化链路
   - 以 `MemberMapper.xml` 达梦实现为行为基准。
   - 迁出旧 CRUD 基类，展开查询 DAO 参数。
   - 仅 `insert` / `update` 使用 `Member` 实体参数。
   - Mapper 保持最小 `BaseMapper<MemberDO>` 定义，业务在 `DaoImpl` 中实现。
   - 用 `DaoImpl` 中的 MyBatis-Plus / Java 持久化实现替代 XML。
   - 消除当前链路 PageHelper 依赖，删除对应 XML，补齐最小验证。
3. 迁移 `Storage` / `StorageBusiness` 持久化链路
   - 以 `StorageMapper.xml` 达梦实现为行为基准。
   - 迁出旧 CRUD 基类，展开查询 DAO 参数。
   - 仅 `insert` / `update` 使用 `Storage` 相关实体参数。
   - Mapper 保持最小 `BaseMapper<StorageDO>` 定义，业务在 `DaoImpl` 中实现。
   - 用 `DaoImpl` 中的 MyBatis-Plus / Java 持久化实现替代 XML。
   - 消除当前链路 PageHelper 依赖，删除对应 XML，补齐最小验证。
4. 迁移 `Log` 持久化链路
