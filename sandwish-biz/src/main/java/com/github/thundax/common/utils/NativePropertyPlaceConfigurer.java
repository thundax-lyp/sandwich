...
public class NativePropertyPlaceConfigurer extends PropertyPlaceholderConfigurer {
    ...
    protected String convertProperty(String propertyName, String propertyValue) {
        ...
        if (propertyValue.startsWith(PREFIX)) {
            try {
                return Sm4Helper.decryptEcb(SALT, propertyValue.substring(PREFIX.length(), propertyValue.length() - 1));
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static void main(String[] args) {
        System.out.println("sa加密结果为：ENC(WDIT:" + Sm4Helper.encryptEcb(SALT, "sa") + ")");
        System.out.println("wdit@123East加密结果为：ENC(WDIT:" + Sm4Helper.encryptEcb(SALT, "wdit@123East") + ")");
    }
}
