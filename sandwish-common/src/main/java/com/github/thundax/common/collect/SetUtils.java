...
public class SetUtils extends org.apache.commons.collections.SetUtils {
    ...
    public static <E> Set<E> newConcurrentHashSet() {
        return Collections.newSetFromMap(new ConcurrentHashMap<>(16));
    }
    ...
    @NonNull
    public static <T, R> Set<R> map(Set<T> set, Function<T, R> action) {
        Set<R> resultSet = new HashSet<>();

        forEach(set, item -> resultSet.add(action.apply(item)));
