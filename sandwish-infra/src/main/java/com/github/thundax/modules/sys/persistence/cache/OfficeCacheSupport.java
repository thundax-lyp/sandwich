...
public class OfficeCacheSupport {
    ...
    public OfficeCacheSupport(RedisClient redisClient) {
        this.redisClient = redisClient;
    }

    public Office getById(String id) {
        return redisClient.get(objectKey(id), Office.class);
    }

    public void putById(Office office) {
        if (office != null && StringUtils.isNotBlank(office.getId())) {
            redisClient.set(objectKey(office.getId()), office, OBJECT_EXPIRE_SECONDS);
        }
    }
